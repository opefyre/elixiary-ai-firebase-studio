...
export async function GET(request: NextRequest) {
  try {
    ...
    const documentation = {
      ...
      baseUrl: 'https://ai.elixiary.com/api/v1',
      authentication: {
        type: 'API Key',
        header: 'x-api-key',
      ...
      examples: {
        curl: {
          getRecipes: `curl -X GET "https://ai.elixiary.com/api/v1/recipes?category=cocktails&limit=5" \\
  -H "x-api-key: elx_live_your_api_key_here" \\
  -H "x-user-email: user@example.com"`,
          saveRecipe: `curl -X POST "https://ai.elixiary.com/api/v1/user/recipes" \\
  -H "x-api-key: elx_live_your_api_key_here" \\
  -H "x-user-email: user@example.com" \\
  ...
  -d '{"recipeId": "abc123"}'`
        ...
        javascript: {
          getRecipes: `const response = await fetch('https://ai.elixiary.com/api/v1/recipes?category=cocktails&limit=5', {
  headers: {
...
response = requests.get('https://ai.elixiary.com/api/v1/recipes?category=cocktails&limit=5', headers=headers)
data = response.json()`
        }
      },
      errorCodes: {
      ...
      support: {
        ...
        documentation: 'https://ai.elixiary.com/api/v1/docs',
