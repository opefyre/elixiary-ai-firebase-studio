...
export async function GET(request: NextRequest) {
  try {
    ...
    const documentation = {
      ...
      baseUrl: `${process.env.NEXT_PUBLIC_APP_URL || 'https://ai.elixiary.com'}/api/v1`,
      authentication: {
        type: 'API Key',
        header: 'x-api-key',
      ...
      examples: {
        curl: {
          getRecipes: `curl -X GET "${process.env.NEXT_PUBLIC_APP_URL || 'https://ai.elixiary.com'}/api/v1/recipes?category=cocktails&limit=5" \\
  -H "x-api-key: elx_live_your_api_key_here" \\
  -H "x-user-email: user@example.com"`,
          saveRecipe: `curl -X POST "${process.env.NEXT_PUBLIC_APP_URL || 'https://ai.elixiary.com'}/api/v1/user/recipes" \\
  -H "x-api-key: elx_live_your_api_key_here" \\
  -H "x-user-email: user@example.com" \\
  ...
  -d '{"recipeId": "abc123"}'`
        ...
        javascript: {
          getRecipes: `const response = await fetch('${process.env.NEXT_PUBLIC_APP_URL || 'https://ai.elixiary.com'}/api/v1/recipes?category=cocktails&limit=5', {
  headers: {
...
response = requests.get('${process.env.NEXT_PUBLIC_APP_URL || 'https://ai.elixiary.com'}/api/v1/recipes?category=cocktails&limit=5', headers=headers)
data = response.json()`
        }
      },
      errorCodes: {
      ...
      support: {
        ...
        documentation: `${process.env.NEXT_PUBLIC_APP_URL || 'https://ai.elixiary.com'}/api/v1/docs`,
